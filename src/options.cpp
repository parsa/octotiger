...
void options::show_help() {
    printf(
        ...
        "-AngCon=<1/0>                         - 1 (default) turns on the angular momentum "
        "conservation feature, 0 turns it off.\n"
        "\n"
        "-Bench                                - Runs for a few time-steps then exit.\n"
        "\n"
        "-Disableoutput                        - Disables SILO and checkpoint output.\n"
        "\n"
        "-Eos=<equation of state>              - Sets the equation of state for pressure and "
        "energy\n"
        ...
        "\n"
        "-Odt=<output frequency>               - Specifies the frequency for SILO output in units "
        "of simulation time.\n"
        "\n"
        "-Omega=<angular frequency>            - The grid rotates along z axis at x=0 and y=0 with "
        "this angular frequency (default 0.0). \n"
        "                                        (Note that for the dwd problem this specifies "
        "only the initial value)\n"
        "\n"
        "-Output=<file name>                   - Load restart file, output to SILO, then exit. "
        "Used for converting checkpoints to SILO.\n"
        "\n"
        "-Datadir=<directory name>             - Directory where to load input from/write output "
        "to (must exist!).\n"
        "\n"
        "-Problem=<problem name>               - Sets up the initial model\n"
        "                                        blast        - Set up and run a Sedov-Taylor "
        "blast wave.\n"
        "                                        dwd          - use SCF solver to find an initial "
        "model for a double white dwarf (see grid_scf.cpp for options). \n"
        "                                                       Terminates after creating restart "
        "file.\n"
        "                                        moving_star  - Set up a single equilibrium star "
        "moving across grid at constant velocity, then run.\n"
        "                                        sod          - Set up and run a Sod shock tube.\n"
        "                                        solid_sphere - Set up a constant density solid "
        "sphere, solve for gravity, then exit.\n"
        "                                        star         - Set up a single equilibrium star "
        "at zero velocity, then run.\n"
        "\n"
        "-Restart=<file name>                  - Restart from a checkpoint file. If this option is "
        "not selected, Octo-tiger sets up the\n"
        "                                        initial problem based on the -Problem option\n"
        "\n"
        "-Stopstep=<stop step>                 - The simulation runs until stop step number of "
        "steps have executed. The firs step from a restart file\n"
        "                                        is counted as step one regardless of the number "
        "of steps prior to the restart (default is infinity).\n"
        "                                        If the simulation time reaches the time specified "
        "by Stoptime first, Octotiger exits then.\n"
        "\n"
        "-Stoptime=<stop time>                 - The simulation runs until the simulation time "
        "hits the time specified here, or until the maximum\n"
        "                                        number of steps specified by Stopstep. (default "
        "is infinity)\n"
        "\n"
        "-VariableOmega=1/0                    - 1 turns on variable omega, 0 off. 0 by default, "
        "unless problem=dwd, then 1 by default\n"
        "                                        mean faster FMM execution but a higher solution "
        "error.\n"
        "-Theta                                - 'Opening criterion' for FMM (default 0.35). Must "
        "be between 1/3 and 1/2, inclusive. Larger values\n"
        "                                        mean faster FMM execution but a higher solution "
        "error.\n"
        "-Xscale=<xmax>                        - The domain of the coarsest grid is set to "
        "(-xmax,xmax) for each all three dimensions (default 1.0)\n"
        "\n"
        "");
}
