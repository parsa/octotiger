...
namespace fmm {
    namespace multipole_interactions {
        ...
        CUDA_CALLABLE_METHOD inline void compute_angular_corrections(T (&X)[NDIM], T (&Y)[NDIM],
            T (&m_partner)[20], T (&tmp_corrections)[3], T (&m_cell)[11], func&& max) noexcept {
            T dX[NDIM];
            dX[0] = X[0] - Y[0];
            dX[1] = X[1] - Y[1];
            dX[2] = X[2] - Y[2];
        ...
        template <typename T, typename func>
        CUDA_CALLABLE_METHOD inline void compute_kernel_angular_corrections(T (&X)[NDIM],
            T (&Y)[NDIM], T (&m_partner)[20], T (&tmp_corrections)[3], T (&m_cell)[11],
            func&& max) noexcept {
            T dX[NDIM];
            dX[0] = X[0] - Y[0];
            dX[1] = X[1] - Y[1];
        ...
        CUDA_CALLABLE_METHOD inline void compute_kernel_non_rho(T (&X)[NDIM], T (&Y)[NDIM],
            ...
            compute_interaction_multipole_non_rho456789(m_partner, tmpstore, D_lower, dX, max);
