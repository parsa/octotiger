...
public:
	...
	hpx::future<hpx::util::tuple<real, op_stats_t<set_basis>>> step(integer) const;
	hpx::future<hpx::util::tuple<real, op_stats_t<set_basis>, diagnostics_t>> step_with_diagnostics(integer,
        const std::pair<space_vector, space_vector>& axis, const std::pair<real, real>& l1, real c1, real c2) const;
	hpx::future<op_stats_t<set_basis>> solve_gravity(bool ene) const;
