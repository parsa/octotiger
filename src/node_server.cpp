...
void node_server::compute_fmm(gsolve_type type, bool energy_account) {
    ...
    // fetch all data before any computation!
    ...
    if (!grid_ptr->get_leaf() && !grid_ptr->get_root()) {
        ...
        std::cout << "old multipole interactions (inner + boundary, ms): " << duration.count()
                  << std::endl;
        total_duration += duration.count();
        total_no_of_called += 1;
        ...
        for (auto const& dir : geo::direction::full_set()) {
            if (!neighbors[dir].empty()) {
                auto tmp = all_neighbor_interaction_data[dir];
                grid_ptr->compute_boundary_interactions(
                    type, tmp.direction, tmp.is_monopole, tmp.data);
            }
        }
    }
    /************************************************************************************************/
