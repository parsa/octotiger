...
void line_of_centers_analyze(const line_of_centers_t& loc, real omega,
    ...
    std::pair<real, real>& l2_phi, std::pair<real, real>& l3_phi, real&, real&);
...
public:
    ...
    bool is_in_star(const std::pair<space_vector, space_vector>& axis,
        const std::pair<real, real>& l1, integer frac, integer index) const;
    ...
    void compute_primitives(const std::array<integer, NDIM> lb = {1, 1, 1},
        const std::array<integer, NDIM> ub = {H_NX - 1, H_NX - 1, H_NX - 1},
        bool tau_only = false) const;
    void set_coordinates();
    void set_hydro_boundary(
        const std::vector<real>&, const geo::direction&, integer width, bool tau_only = false);
    std::vector<real> get_hydro_boundary(
        const geo::direction& face, integer width, bool tau_only = false);
    ...
    void compute_interactions_inner(gsolve_type);
    void compute_interactions_leaf(gsolve_type);
