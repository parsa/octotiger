...
namespace fmm {
    ...
    constexpr uint64_t EXPANSION_COUNT_PADDING =
        2 * DIMENSION * PADDING_EDGE_SURFACE_SINGLE_DIRECTION * INNER_CELLS_PADDING_DEPTH +
        detail::const_pow(2, DIMENSION) * detail::const_pow(INNER_CELLS_PADDING_DEPTH, DIMENSION);
    constexpr uint64_t EXPANSION_COUNT_NOT_PADDED = INNER_CELLS;
    constexpr uint64_t EXPANSION_COUNT_PADDED = EXPANSION_COUNT_PADDING + EXPANSION_COUNT_NOT_PADDED;
