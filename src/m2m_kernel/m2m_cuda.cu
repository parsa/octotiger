...
__global__ void trivial_kernel(double *local_expansions_SoA_ptr,
			       double *center_of_masses_SoA_ptr,
			       double *potential_expansions_SoA_ptr,
			       double *angular_corrections_SoA_ptr) {
  //TODO: make sure you pick up the right expansion type
  octotiger::fmm::cuda::struct_of_array_data<expansion, real, 20, octotiger::fmm::ENTRIES,
				       octotiger::fmm::SOA_PADDING> local_expansions_SoA(local_expansions_SoA_ptr);
...
namespace octotiger {
  namespace fmm {
    namespace cuda {
      ...
      // 	  m2m_vector tmp = potential_expansions_SoA.value<0>(cell_flat_index_unpadded) + cur_pot[0];
      // 	  Vc::where(mask, tmp).memstore(
      // 					potential_expansions_SoA.pointer<0>(cell_flat_index_unpadded),
      // 					Vc::flags::element_aligned);
      ...
      void m2m_cuda::compute_interactions(
					  ...
					  octotiger::fmm::struct_of_array_data<expansion, real, 20, ENTRIES,
					  SOA_PADDING> &potential_expansions_SoA,
					  octotiger::fmm::struct_of_array_data<space_vector, real, 3, ENTRIES, SOA_PADDING> &angular_corrections_SoA) {

	cuda_buffer d_local_expansions_SoA  = octotiger::fmm::cuda::move_to_device(local_expansions_SoA);
	cuda_buffer d_center_of_masses_SoA  = octotiger::fmm::cuda::move_to_device(center_of_masses_SoA);
	cuda_buffer d_potential_expansions_SoA  = octotiger::fmm::cuda::move_to_device(potential_expansions_SoA);
	cuda_buffer d_angular_corrections_SoA  = octotiger::fmm::cuda::move_to_device(angular_corrections_SoA);
