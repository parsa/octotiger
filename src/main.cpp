...
int hpx_main(int argc, char* argv[]) {
    ...
    try {
        if (opts.process_options(argc, argv)) {
                ...
                if (opts.output_only) {
                    ...
                    if (opts.max_restart_level > 0) {
                        for (integer l = 0; l < opts.max_restart_level - 1; ++l) {
                            ngrids =
                                root->regrid(root_client.get_gid(), grid::get_omega(), -1, false);
                            printf(
                                "---------------Created Level %i---------------\n\n", int(l + 1));
                        }
                    }
            ...
            if (!opts.output_only) {
                //  set_problem(null_problem);
                hpx::async(&node_server::start_run, root,
                    opts.problem == DWD && !opts.found_restart_file, ngrids)
                    .get();
                //              root->start_run(opts.problem == DWD && !opts.found_restart_file,
                //              ngrids);
            }
            root->report_timing();
        }
